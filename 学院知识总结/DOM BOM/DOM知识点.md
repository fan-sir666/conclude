...
<body>
    ...
    <script>
        // 获取所需元素
        let aInput = document.querySelectorAll('#check input');
        ...
        /* for (let j = 0; j < aItem.length; j++) {
            ...
            aItem[j].onclick = function() {
                // 假设每个商品复选框都钩上了 声明一个变量flag代表一个状态 
                let flag = true;
                // 假设其中有一个复选框没有钩上 咋办 那就把flag的true改成false
                for (let k = 0; k < aItem.length; k++) {
                    if (!aItem[k].checked) {
                        flag = false;
                        // 点击一次 129的for循环会把每个商品项都循环一次 所以当只要发现有一个没有被选中 就可以直接停止循环了 这样可以减少循环次数
                        break;
                    }
                }
                // 如果每个复选框的对号都钩上了 全选的复选框 应该也钩上
...
<body>
    ...
    <script>
        // 代码的逻辑: 
        /* 获取相关的两个伪数组, 循环遍历点击切换数组 注册点击事件并将 索引值通过操作自定义属性的方式 setAttribute 方法设置到元素上
        第一次排他来添加 点击元素上的css效果（ 排他先全部移除相关效果的类名， 再添加当前元素的效果）
        第二次排他 先将自定义的索引值获取到getAttribute 再对关联的数组进行排他 最后显示索引对应的数组内容 */
        // 1.1 获取ul中的li
        let aLi = document.querySelectorAll('li');
        let aItem = document.querySelectorAll('.item');
        // 1.2 遍历ul中的li
        for (let i = 0; i < aLi.length; i++) {
            // 2.1 在遍历的时候 顺便把li的索引添加为自定义属性
            aLi[i].setAttribute("index", i);
            // 1.3 给每个li添加点击事件
            aLi[i].onclick = function() {
                // 1.4 排他: 清除所有li身上的current类名
                for (let j = 0; j < aLi.length; j++) {
                    aLi[j].classList.remove('current');
                }
                // 1.5 排他: 给点击的那个li添加current类名
